...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, onLogout }) => {
  const navItems = [
    ...
    { id: 'DAILY_LOG' as ViewType, label: 'Daily Log', icon: ICONS.DailyLog },
      ...
      <nav className="flex-1 px-4 py-4 space-y-1">
        {navItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${activeView === item.id
